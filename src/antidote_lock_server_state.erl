...
-record(state, {
    ...
    remote_requests = [] :: [{Requester :: dcid(), lock_request_actions_for_dc()}],
    % remote requests that already have been fulfilled
    % (might be removed from here if additional permissions come in)
    answered_remote_requests = [] :: ordsets:ordset(lock_request_actions_for_dc()),
...
-type lock_request_actions_for_dc() :: {pid(), RequestTime :: milliseconds(), antidote_locks:lock_spec_item()}.
...
-record(actions, {
    ...
    ack_locks = [] :: [{pid(), antidote_locks:lock_spec_item()}]
}).

-type actions() :: #actions{}.


% TODO eventually change actions to a more general set of actions:
%%-type action() ::
%%    {read_crdt_state, snapshot_time(), [bound_object()], read_crdt_state_onresponse_data()}
%%  | {update_crdt_state, snapshot_time(), [{bound_object(), op_name(), op_param()}], update_crdt_state_onresponse_data()}
%%  | {send_inter_dc_message, Receiver :: dcid(), Message :: any()}
%%  | {accept_request, requester()}
%%  | {abort_request, requester()}
%%  | {set_timeout, milliseconds()}.
%%
%%-opaque read_crdt_state_onresponse_data() :: any().
%%-opaque update_crdt_state_onresponse_data() :: any().
%%
%%-type actions() :: [action()].
...
new_request(Requester, RequestTime, SnapshotTime, AllDcIds, LockEntries, State) ->
    ...
    debug_result(case MissingLocks of
        ...
        _ ->
            ...
            LockRequestActions = [{RequesterPid, RequestTime2, L} || L <- MissingLocks],
            RequestsByDc2 = [{Dc, LockRequestActions} || Dc <- OtherDcs],
...
new_remote_request(CurrentTime, LockRequestActions, State) ->
    ...
    NewState = State#state{
        remote_requests          = [LockRequestActions | State#state.remote_requests],
        answered_remote_requests = ordsets:subtract(State#state.answered_remote_requests, [LockRequestActions])
    },
...
retries_for_waiting_remote(Time, RetryDelta, MyDc, OtherDcs, State) ->
    WaitingRemote = [{Pid, PidState#pid_state.request_time, [{L, K} || {L, {waiting_remote, K}} <- PidState#pid_state.locks]} ||
        {Pid, PidState} <- maps:to_list(State#state.by_pid)],
    WaitingRemoteLong = [{Pid, T, L} || {Pid, T, L} <- WaitingRemote, T + RetryDelta =< Time],
    ...
    LockRequests = [{Dc, ByLock} || ByLock /= [], Dc <- OtherDcs],
...
print_state(State) ->
    #{
        ...
        remote_requests => lists:map(fun({Pid, T, L}) -> {Pid, print_systemtime(T), L} end, State#state.remote_requests),
...
handle_remote_requests(State, CurrentTime) ->
    ...
    LocksToTransfer = lists:filter(fun(Blub) ->
        {Dc,Req} = Blub,
        {_Pid, T, {L, K}} = Req,
        true
...
%%            andalso not exists_conflicting_remote_request(Dc, L, K, T, State)
    ...
    Locks = [L || {_Dc, {_Pid, _T, {L, _K}}} <- LocksToTransfer],


    % check local requests and get the locks that must be requested again
    RequestAgainLocal = get_local_waiting_requests(LocksToTransfer, State),
    ...
    HandoverActions = maps:map(
        ...
        antidote_list_utils:group_by_first([{Dc, {L, K}} || {Dc, {_Pid, _T, {L, K}}} <- LocksToTransfer])),
...
merge_actions(A1, A2) ->
    #actions{
        ...
        lock_request = A1#actions.lock_request ++ A2#actions.lock_request,
...
shared_lock_fail_test() ->
    ...
    ?assertEqual(#{}, Actions#actions.hand_over),
    ?assertEqual(#{dc2 => #{{dc1, lock1} => {shared, 11}}, dc3 => #{{dc1, lock1} => {shared, 11}}}, Actions#actions.lock_request),
...
shared_lock_missing_local_test() ->
    ...
    ?assertEqual(#actions{lock_request = #{dc2 => #{{dc1, lock1} => {shared, 11}}, dc3 => #{{dc1, lock1} => {shared, 11}}}}, Actions1),

    % later we receive the lock from dc3 and we can get the lock
    {Actions2, _S2} = on_remote_locks_received(99, #{}, [dc1, dc2, dc3], [{{lock1, shared}, #{dc1 => dc1, dc2 => dc1, dc3 => dc3}}], S1),
...
shared_locks_missing_local_test() ->
    ...
    ?assertEqual(#actions{lock_request = #{dc2 => #{{dc1, lock2} => {shared, 11}}, dc3 => #{{dc1, lock2} => {shared, 11}}}}, Actions1),
...
exclusive_lock_fail_test() ->
    ...
    ?assertEqual(#{}, Actions#actions.hand_over),
    ?assertEqual(#{dc2 => #{{dc1, lock1} => {exclusive, 11}}, dc3 => #{{dc1, lock1} => {exclusive, 11}}}, Actions#actions.lock_request),
    ?assertEqual([], Actions#actions.replies),
    ok.
...
same_time_remote_local_test() ->
    SInit = initial(dc1),
    % first R1 tries to acquire lock1
    R1 = {p1, t1},
    {Actions1, S1} = new_request(R1, 10, vectorclock:new(), [dc1, dc2, dc3], [{{lock1, exclusive}, #{}}], SInit),
    ?assertEqual(#actions{lock_request = #{dc2 => #{{dc1, lock1} => {exclusive, 11}},
        dc3 => #{{dc1, lock1} => {exclusive, 11}}}}, Actions1),

    % then remote DC2 tries to acquire the same lock
    {Actions2, S2} = new_remote_request(11, #{{dc2, lock1} => {exclusive, 20}}, S1),
    ?assertEqual(#actions{}, Actions2),

    % and DC3 tries with the same timestamp
    {Actions3, S3} = new_remote_request(11, #{{dc3, lock1} => {exclusive, 11}}, S2),
    ?assertEqual(#actions{}, Actions3),

    % after timeout, DC3 should not get the lock
    {Actions4, _S4} = timer_tick(S3, 99),
    ?assertEqual(#actions{}, Actions4),
    ok.

same_time_remote_local2_test() ->
    SInit = initial(dc3),
    % first R1 tries to acquire lock1
    R1 = {p1, t1},
    {Actions1, S1} = new_request(R1, 10, vectorclock:new(), [dc1, dc2, dc3], [{{lock1, exclusive}, #{}}], SInit),
    ?assertEqual(#actions{lock_request = #{dc1 => #{{dc3, lock1} => {exclusive, 11}},
        dc2 => #{{dc3, lock1} => {exclusive, 11}}}}, Actions1),

    % then remote DC2 tries to acquire the same lock
    {Actions2, S2} = new_remote_request(10, #{{dc2, lock1} => {exclusive, 20}}, S1),
    ?assertEqual(#actions{}, Actions2),

    % and DC1 tries with the same timestamp
    {Actions3, S3} = new_remote_request(10, #{{dc1, lock1} => {exclusive, 11}}, S2),
    ?assertEqual(#actions{}, Actions3),

    % after timeout, DC1 should get the lock
    {Actions4, _S4} = timer_tick(S3, 99),
    ?assertEqual(#actions{
...
missing_locks_empty_test() ->
    OwnsLock = owns_lock([dc1, dc2, dc3], dc1, exclusive, #{}),
    ?assertEqual(false, OwnsLock),
    ok.

missing_locks_by_dc_empty_test() ->
    Missing = missing_locks([dc1, dc2, dc3], dc1, [{{lock1, exclusive}, #{}}]),
    ?assertEqual([{lock1, exclusive}], Missing),
    ok.
...
locks_missing_local_retry_test() ->
    ...
    ?assertEqual(#actions{lock_request =
    #{
        dc2 => #{{dc1, lock1} => {shared, 11}, {dc1, lock2} => {exclusive, 11}, {dc1, lock3} => {shared, 11}},
        dc3 => #{{dc1, lock1} => {shared, 11}, {dc1, lock2} => {exclusive, 11}, {dc1, lock3} => {shared, 11}}
    }},
        Actions1),
    ...
    ?assertEqual(#actions{lock_request = #{
        ...
        dc3 => #{{dc1, lock1} => {exclusive, 21}, {dc1, lock4} => {shared, 21}}}}, Actions2),
